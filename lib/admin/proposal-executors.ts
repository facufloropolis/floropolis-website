...
// ---------------------------------------------------------------------------
// price.formula_deviation_review — resets mirror price to formula-computed price
// Generated by Rose_BI from ops.price_audit_log FAIL rows (2026-05-27 audit).
// Identical pattern to execPriceFormulaReset; kept separate for audit clarity.
// ---------------------------------------------------------------------------

async function execPriceFormulaDeviationReview(
  proposal: AdminProposal,
  service: SupabaseClient,
): Promise<ExecutorResult> {
  if (!proposal.target_id) return fail('missing_target_id');
  ...
  const payload = payloadObject(proposal);
  if (!payload) return fail('invalid_payload');
  const formulaPrice = payload.formula_price ?? payload.expected_price;
  if (formulaPrice === undefined || formulaPrice === null) return fail('missing_formula_price_in_payload');
  const newPrice = typeof formulaPrice === 'number' ? formulaPrice : parseFloat(String(formulaPrice));
  ...
  return {
    ...
    auditEntries: [{
      ...
      after_jsonb: {
        ...
        source: 'price.formula_deviation_review',
        deviation_pct: payload.deviation_pct ?? null,
      },
      applied_by_function: 'proposal-executors.execPriceFormulaDeviationReview',
    }],
  };
}
...
export async function executeProposal(
  ...
  switch (proposal.type) {
    ...
    case 'price.formula_deviation_review':
      return execPriceFormulaDeviationReview(proposal, service);
...
export const KNOWN_PROPOSAL_TYPES: readonly string[] = [
  ...
  'price.formula_reset',
  // Formula deviation individual review (2026-05-27 audit): Rose-generated proposals from
  // ops.price_audit_log FAIL rows. Each proposal resets one SKU to its formula-computed price.
  'price.formula_deviation_review',
