...
export const ADMIN_WIRING: WiringPageEntry[] = [
  ...
  },
  {
    ...
    sections: [
      { id: 'summary-tiles', level: 'LIVE', note: 'Collapsed into header subtitle line per mockup (SKUs / vendors / countries / K2K live / T2 / T3 / live / hidden / draft). Universe count = T2 + T3 + K2K live (de-duped).' },
      ...
      { id: 'tabs', level: 'LIVE', note: 'Improvement queue (default, priority_to_fix DESC, excludes Perfect) / Perfect / All. Counts driven by buildCatalog output.' },
      { id: 'filter-form', level: 'LIVE', note: 'Mockup-parity row: search + vendor + source + category + visibility + GPM band + flags. All filters drive URL searchParams; server reads them and re-renders.' },
      { id: 'catalog-table', level: 'LIVE', note: 'Mockup-parity 12-column table per BRD UC-104. Columns: SKU / Quality Family / Vendor / Box / Cost / Delivery (per-stem shipping cost) / Price / GPM / Sources / Avail / Visibility (with quality_score badge inline) / Flags (with importance star badge below SKU name).' },
      { id: 'bulk-actions', level: 'PLAN', note: 'Bulk override-price / change-vendor / add-to-campaign buttons rendered disabled until executors exist. Bulk selection (per-row checkboxes) deferred until a Context-based island wraps the table.' },
      { id: 'pagination', level: 'LIVE', note: '100 rows per page (UC-104 acceptance). ?page=N URL param.' },
      { id: 'export-csv', level: 'LIVE', note: '/api/admin/catalog/export stream link unchanged from v4.' },
    ],
  },
  {
