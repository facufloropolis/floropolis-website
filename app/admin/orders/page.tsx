...
// v2 | 2026-05-19 | Job_PM admin-port ORD [V8 SHADOW]
//
// Three tabs:
//   1. Web orders (D1)   -- ?tab=d1 (default)
...
type TabId = 'd1' | 'k2k_orders' | 'k2k_prebooks';
...
function isTabId(v: string | undefined): v is TabId {
  return v === 'd1' || v === 'k2k_orders' || v === 'k2k_prebooks';
}
...
interface PageProps {
  searchParams: Promise<{
    tab?: string;          // d1 | k2k_orders | k2k_prebooks (default d1)
...
function buildHref(
  ...
  if (params.tab && params.tab !== 'd1') sp.set('tab', params.tab);
...
export default async function AdminOrdersPage({ searchParams }: PageProps) {
  ...
  const tab: TabId = isTabId(sp.tab) ? sp.tab : 'd1';
      ...
      <main className="max-w-7xl mx-auto px-4 py-10">
        ...
        <div className="flex gap-1 border-b border-slate-200 mb-6 overflow-x-auto">
          {TAB_DEFS.map((t) => {
            ...
            const href = t.id === 'd1' ? '/admin/orders' : `/admin/orders?tab=${t.id}`;
            return (
              <Link
                key={t.id}
                href={href}
...
function FilterForm({
  ...
  return (
    ...
    >
      {/* Preserve tab through form submit (D1 default has no hidden field). */}
      {tab !== 'd1' && <input type="hidden" name="tab" value={tab} />}
