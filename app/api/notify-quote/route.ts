...
const FACU_EMAIL = "facu@inhauslab.com";
const JJ_EMAIL = "jj@inhauslab.com";
const WHATSAPP_NUMBER = "14153352771";
...
function formatItemsText(items: QuoteItem[]): string {
  return items
    .map((item) => {
      ...
      const lineTotal = price * item.quantity * item.units_per_box;
      return `- ${item.name} (${item.category}) | ${item.stem_length || ""} | ${item.quantity}x ${item.box_type} (${item.units_per_box} ${item.unit === "Bunch" ? "bunches" : "stems"}) | $${price.toFixed(2)}/${item.unit.toLowerCase()} | $${lineTotal.toFixed(2)}${item.delivery_date ? ` | Delivery: ${item.delivery_date}` : ""}`;
    })
    .join("\n");
}
...
async function sendBrevoEmail(payload: QuotePayload, quoteId: number | null): Promise<boolean> {
  ...
  try {
    const res = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        sender: { name: "Floropolis Orders", email: "orders@floropolis.com" },
        to: [
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 3. Build WhatsApp URL for client
    ...
    // Log result
    console.log(`[Quote] Saved: ${quoteId ? `#${quoteId}` : "FAILED"} | Email: ${emailSent ? "SENT" : "FAILED"} | ${payload.business_name} | $${payload.total.toFixed(2)}`);
