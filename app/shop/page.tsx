      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-8">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* EXP-067: Rich empty state — WhatsApp shortcut pre-filled with search query */}
            {sortedGroups.length === 0 && (
              <div className="text-center py-16">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  {searchQuery.trim()
                    ? "We source 270+ varieties — ask us directly if you don\'t see it."
                ...
                <div className="mt-5 flex flex-col sm:flex-row gap-3 justify-center">
                  {searchQuery.trim() ? (
                    <>
                      <a
                        href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(`Hi! I\'m looking for ${searchQuery.trim()} — do you carry it?`)}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={() => pushEvent("custom_request_whatsapp_click", { source: "empty_search", query: searchQuery.trim() })}
                        className="inline-flex items-center justify-center gap-2 bg-emerald-600 text-white text-sm font-semibold px-5 py-2.5 rounded-lg hover:bg-emerald-700 transition-colors"
                      >
                      ...
                      <Link
                        href="/quote"
                        className="inline-flex items-center justify-center gap-1.5 border border-slate-300 text-slate-700 text-sm font-semibold px-5 py-2.5 rounded-lg hover:bg-slate-50 transition-colors"
                      >
