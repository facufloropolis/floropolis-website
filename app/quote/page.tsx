            ...
            {items.length === 0 ? (
              <div className="py-10 space-y-6">
                ...
                <p className="text-center text-sm text-slate-500">
                  ...
                  <a
                    href="https://wa.me/16452405203?text=Hi!%20I%27d%20like%20help%20putting%20together%20a%20wholesale%20flower%20order."
                    target="_blank"
            ...
            <div className="sticky top-24 bg-slate-50 rounded-2xl border border-slate-200 p-6">
              ...
              <form id="quote-form" onSubmit={handleSubmit} className="space-y-3 text-sm"
                ...
                {!isExistingClient && (
                  <div className="space-y-3 pt-2 border-t border-slate-200">
                    <button
                      ...
                      className="text-xs text-slate-500 hover:text-emerald-600 transition-colors flex items-center gap-1"
                    >
                      {showShipping ? "▾" : "▸"} Add shipping address{" "}
                      <span className="text-slate-400">(optional — we'll confirm on callback)</span>
                    </button>
                    {showShipping && <>
                    <div>
                      <input
                ...
                <div className="grid grid-cols-3 gap-1 text-center text-[10px] text-slate-500 py-2 border-t border-slate-100">
                  <div>
                    <div className="font-semibold text-slate-700">No minimum</div>
                ...
                {/* EXP-131: Process explanation — reduce anxiety about what happens after submit */}
                <p className="text-xs text-slate-500 text-center bg-slate-50 rounded-lg px-3 py-2.5">
                  This is a custom order — we&apos;ll confirm availability and contact you within 2 hours.
                </p>
                <p className="text-[11px] text-emerald-700 text-center font-medium -mt-1">
                  ✓ We respond within 2 hours (Mon–Fri, 8–6 ET)
                </p>
                ...
                {/* EXP-056+081+132: WhatsApp as equal-weight alternative — full-width on mobile */}
                {items.length > 0 && (
                  <div className="pt-2 border-t border-slate-100">
                    <p className="text-[11px] text-center text-slate-400 mb-2">Prefer to chat?</p>
                    <a
                      href={buildWhatsAppQuoteUrl()}
                      target="_blank"
                      rel="noopener noreferrer"
                      onClick={() => pushEvent(CTA_EVENTS.quote_whatsapp_click, { item_count: items.length })}
                      className="w-full flex items-center justify-center gap-2 rounded-xl border-2 border-[#25D366] text-[#128C2E] font-bold text-sm px-4 py-3 hover:bg-[#f0fff4] transition-colors"
                    >
